...
public class SwerveDrive {
	...
	public void updateSwerve(double forward, double strafe, double rotIn) {
		...
		while (Math.abs(oldTopRightAngle - topRightAngle) > 90) {
		 ...
		 // bl
		 bottomLeftAngle += 360 * (int) (oldBottomLeftAngle - bottomLeftAngle);
		 ...
		 // br
		 bottomRightAngle += 360* (int) (oldBottomRightAngle - bottomRightAngle);
...
//		}
		 ...
		 * s2)); } else { forward *= -1; }
		 * 
		 * if (isFieldcentric == false) { rotIn *= 2 * Math.PI; } else { heading
		 * = rotIn;
		 * 
		 * // defining the velocity vector in polar to convert to // robocentric
		 * double magnitudeV = Math.sqrt(Math.pow(strafe, 2) + Math.pow(forward,
		 ...
		 * lastHeading) / .02; // the code should be updated to the robot every
		 * 20ms, so // hardcoded
		 * 
		 * lastHeading = heading; }
		 * 
